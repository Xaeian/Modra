...
//   stack.build();
//   stack.setData(0, [ts[], v1[]]);
//   stack.setXRange(min, max);
//   stack.resetZoom();
//   stack.destroy();
...
// ---------------------------------------------------------------- Tooltip plugin (legendAsTooltip pattern)
...
const ChartStack = class {
  ...
  build() {
    ...
    for(let i = 0; i < n; i++)
      this._buildOne(i, i === n - 1);
  }

  _currentRange() {
    if(!this._autoScroll && this._zoomMin != null)
      return [this._zoomMin, this._zoomMax];
    if(this._xMin != null)
  ...
  _buildTipHTML(u, idx) {
    ...
    let html = '<div class="cs-tip-ts">' +
      (this._formatXValue ? this._formatXValue(ts) : String(ts)) + '</div>';
    for(let p = 0; p < this._entries.length; p++) {
      ...
      for(let s = 0; s < panel.series.length; s++) {
        ...
        html += '<div class="cs-tip-row">' +
          ...
          '<span class="cs-tip-val">' + (panel.yFormat || csFmtVal)(val) +
          (panel.unit ? ' ' + panel.unit : '') + '</span></div>';
      }
    }
    return html;
  }
