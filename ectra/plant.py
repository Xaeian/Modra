...
# Stages where the `Volt` table drives the bridge, so voltage is imposed and
# current is whatever the machine draws. Everywhere else the loops impose
# current and the voltage is whatever it takes to push it.
TABLE_VEC = VERDICT_VEC + ("catch",)

# What the current-vector estimator recovers of the truth. Every harmonic drags
# the `|I|^2`-weighted average down, and an open-loop table carries the most.
ESTIM_TABLE = 0.58
ESTIM_VECTOR = 0.85
ESTIM_FLOOR_A = 0.05
ESTIM_FLOOR_Hz = 0.5
...
class Plant:
  ...
  def reset(self):
    ...
    self.meters.reset()
    self.scope.reset()
    # Prime both tiers off the cold machine, so a read that beats the first
    # motor tick still gets an ambient stage and a charged link rather than
    # zeros standing in for them.
  ...
  def _clear_run(self):
    """Everything one run owns. A commanded stop, a shutdown at the cutoff and a
    cleared fault all land here: the bridge is off and nothing is carried over.
  ...
  def estim_hz(self) -> float:
    """The current-vector measurement, independent of the observer and of
    nothing else: under the vector core the loops impose the angle, so it
    witnesses the vector; under a table the BEMF currents turn it, so it
    witnesses the rotor. Neither reads its own domain at face value."""
  ...
  def report_hz(self) -> float:
    """What the measurement tiers publish while the drive runs: the control
    domain, never the estimator. Standstill and coast fall back on the shaft, so
    a rotor still turning stays visible."""
  ...
  @property
  def vd(self) -> float:
    """Flux axis as a share of the modulation, which is what the register
    carries: a percentage of the bridge, not volts."""
    return self._axis(0)
  ...
  def step(self, cmd:Command, dt:float):
    ...
    if cmd.clear_fault and self.fault:
      # A cleared fault is the bridge coming back, not the run resuming. The
      # coast still has to finish; the drive starts again if a setpoint stands.
      self.fault = None
      self.trip = {}
    ...
    if self.vec == "idle":
      # Stopped, but the shaft may still be turning and the link still holds
      # charge: both keep draining.
      self._idle(cmd, dt)
    ...
    self._detect(cmd, dt)
    self._settle(cmd, dt)
    # `RENDER_Emergency` runs in the pass that tripped: the bridge is down
    # before the next tick, so a clear can never land on a still-running fault.
    if self.fault:
      self._latch(cmd)
      return
    self._cutoff(cmd)
  ...
  def _idle(self, cmd:Command, dt:float):
    """Bridge off, but the world does not stop with it, and the boot deadline
    keeps running: an idle drive is exactly where the device spends it."""
    self.machine.coast(dt)
    self._settle(cmd, dt)

  def _settle(self, cmd:Command, dt:float):
    """The link, the senses and the protections: everything that follows from an
    operating point rather than deciding one."""
    running = self.vec not in ("idle", "fault")
    # Root-cause latch: a secondary trip during the coast (regen `hv+`) must not
    # overwrite the fault that started it, as `RENDER_Emergency` refuses to.
    ...
    # Every threshold reads the fast tier, because on the device that is the
    # only layer a protection is allowed to see.
  ...
  def _coast(self, cmd:Command, dt:float):
    """The bridge is down. The rotor spins out under its own load and the drive
    parks in Shutdown only once the quoted coast has run, so a fault cleared
    early cannot re-arm into a turning machine. A fault landing mid-coast just
    stands as the code: the coast clock is never restarted."""
  ...
  def _verdict(self, cmd:Command) -> str|None:
    """Whether a vector may run at all, in the firmware's own terms and in its
    own order. `Motor:Rs` is the current loop's integral, so zero sheds the
    vector in every mode; `Motor:Ke` only feeds the observer, so `if` runs
    happily without it."""
    if cmd.mode == "vf": return "off"
    if not cmd.shunt_ok: return "shunt"
    # The ceiling answers before the observer does, as `render.c` orders it: no
    # voltage authority is a reason on its own, whatever the model knows.
    ...
    # A sense chain wired against the map cannot be told from a dead one until
    # current is asked for, which is what the start feed guard is for.
    if cmd.phasemap != self.machine.phasemap: return "guard"
    return None

  def _bus_ok(self, cmd:Command) -> bool:
    """The DC-bus window, with the bounds independent and an upper bound of zero
    disabling itself, as `RENDER_Setpoint` tests it."""
  ...
  def _boot(self, cmd:Command, dt:float):
    ...
    The device polls `RENDER_Setpoint` on every link frame while `Ctrl:Mode` is
    not `off`, setpoint or no setpoint, and arms the deadline the first time it
    sees the bus in window. By the time an operator writes a speed it is long
    gone. Charging it at the start instead adds the whole delay to the first run
    of every session, which is not a stabilisation and is not what the register
    buys. Only a bus excursion re-arms it."""
  ...
  def _start(self, cmd:Command):
    """The rotor is pulled to a known angle before rotation begins, because a
    random-angle catch is start-or-stall. `If:CatchFreq` starts on the V/f table
    and catches the vector there instead."""
    if self.vec != "idle" or not self._armed(cmd): return
    self.ramp.hz = cmd.init_hz
    self.phase_deg = 0.0
    self.scope.restart(self.machine.vdc)
    # Every start spends the align window in `Start`: `RENDER_Run` is a timer
    # task, so the ramp waits it out on the catch and verdict paths too. Only
    # the align STAGE freezes the field; the others render at `Drive:InitFreq`.
  ...
  def _mode_change(self, cmd:Command):
    ...
    self.det.restart()
    verdict = self._verdict(cmd)
    # The ceiling gates the ENTRY too: catching a vector above it would strand
    # the drive up there, since the hold only ever stops a climb.
    if cmd.max_freq_hz and self.ramp.hz > cmd.max_freq_hz: verdict = "volts"
  ...
  def _cutoff(self, cmd:Command):
    ...
    rather than crawling to zero: below the cutoff there is nothing left worth
    rendering, so the drive drops the bridge."""
  ...
  def _rebase(self):
    """Keep the physical vector continuous across a stage change. Whoever takes
    the bridge next renders a different quantity, so the angle has to be
    restated in its terms before the first sub-step of the new stage."""
    table = self.table
  ...
  def _output(self, cmd:Command, dt:float):
    ...
    A drive cannot hold voltage and current at once, so publishing both from
    their own tables would describe hardware that does not exist. Under the
    `Volt` table the bridge sets volts and the current is whatever the winding
    draws against its back-EMF; under a current vector the loops set amps and
    the voltage is whatever it takes to push them past that same back-EMF.

    Either way the machine integrates it. What arrives here is a job, not an
    answer, which is the only reason a step in the command looks like one."""
    self._rebase()
    m, hz = self.machine, self.ramp.hz
    closed = self.vec == "closed"
    # Align freezes the rendered angle, so the field waits and the rotor pulls
    # into it. A closed loop rides the observer, so the field IS the rotor and
    # the load angle stops being a free state.
    ...
    # Only the vector core is bound by the register's circle. The table is bound
    # by the bridge, and clamping it early would hide a starved table.
  ...
  def _detect(self, cmd:Command, dt:float):
    """Hand the elapsed time to the detector and apply what it decided. The
    stage transitions the detector cannot see belong here: a verdict voids the
    evidence, and align and catch both end in the forced vector."""
    if self.vec in VERDICT_VEC:
      self.det.lock = 0
      self.det.close_done = False
      return
    if self.vec == "align" and self._align_s <= 0.0: self.vec = "forced"
    # A catch armed by a start waits for its frequency; one armed by a live mode
    # change has no rising edge to wait for and takes the current one.
