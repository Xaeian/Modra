...
const EC_STAGES = [
  ...
  { id: "dane", label: "1 Dane",
    ...
    text:
      ...
      + "100 Hz do 1 kHz: obróć wał, znajdź największy odczyt, a wynik również podziel "
      + "przez dwa. "
      + "Motor:PolePairs to liczba par biegunów, nie liczba biegunów. Motor:Ke podaj "
      + "jako międzyfazowe napięcie RMS przy 1000 rpm, na podstawie danych znamionowych "
      ...
      + "wartość zadana jest niezerowa, a odczyt Foc:GuardId pozostaje bliski zera, "
      + "sprawdź okablowanie i ciągłość kanału pomiarowego. Przeciwny znak wskazuje odwróconą "
  ...
  { id: "if", label: "3 I/f",
    ...
    text:
      ...
      + "\n"
      + "W trybie I/f Obs:AngleErr pokazuje kąt między wymuszonym wektorem prądu a "
      + "położeniem wirnika. Wartość zbliżona do 90° oznacza, że pozostaje niewielki "
      + "zapas momentu, a przejęcie może być niestabilne. W okolicy planowanej "
      + "częstotliwości przejęcia zwiększaj odpowiednie punkty tabeli Curr, aż kąt "
      + "będzie wyraźnie mniejszy niż 90°. W pozostałym zakresie ustaw najmniejszy "
      + "prąd, przy którym silnik nie wypada z synchronizmu."
      + "\n"
      + "Zabezpieczenie spin zapamiętuje moc pozorną przy 15 Hz, a powyżej 40 Hz "
      + "sprawdza, czy moc wzrosła wystarczająco. Jest aktywne tylko wtedy, gdy "
      + "Curr:40Hz nie jest mniejsze niż Curr:15Hz; przy opadającym profilu pozostaje "
      + "wyłączone, aby nie zgłaszać fałszywego błędu. Po zmianie prądu w okolicy "
      + "przejęcia wykonaj pełny rozruch i sprawdź, czy nie pojawia się błąd spin."
      + "\n"
  ...
  { id: "obs", label: "4 Obserwator",
    find: "Obs:|Pll:",
    mute: EC_MUTE,
    plot: ["Obs:Bias", "Sync:ErrPeak", "Obs:OmegaHat", "Obs:AngleErr"], big: ["Obs:Bias"],
    text:
      ...
      + "Feedback:RenderFreq. Zapisuj Sync:ErrPeak i Obs:AngleErr dla każdej badanej "
      + "częstotliwości. Nie wybieraj punktu przejęcia wyłącznie na podstawie minimum "
      + "Sync:ErrPeak: szczyt rośnie z prądem, więc takie kryterium faworyzuje małą "
      + "częstotliwość i mały prąd. Wybierz najwyższą częstotliwość, przy której szczyt "
      + "jest tylko o kilka stopni większy od minimum, a kąt obciążenia pozostaje "
      + "wyraźnie mniejszy niż 90°."
      + "\n"
      ...
      + "zakresie, wróć do sprawdzenia Motor:Ke, Sense:PhaseMap i skali pomiaru prądu. "
      + "Nie zwiększaj tolerancji warunków przejęcia, aby ukryć błędną estymację."
      + "\n"
      ...
      + "i zwykle obniża Sync:ErrPeak. Zwiększaj wartość stopniowo. Gdy Obs:Bias zacznie "
      + "rosnąć, nieco zmniejsz Pll:Bw, aby ustabilizować estymatę. Szerokie pasmo "
      + "przenosi tętnienie na chwilową prędkość obserwatora. Modbus pokazuje tylko jej "
      + "wartość uśrednioną, natomiast bramka przejęcia porównuje wartość chwilową, więc "
      + "Foc:LockSpeed musi uwzględniać te odchyłki. Wartość Pll:Damp zbliżona do 1 "
      + "tłumi szczyt; mniejsze wartości go zwiększają."
      + "\n"
  ...
  { id: "sync", label: "5 Przejęcie",
    find: "Foc:Entry|Foc:Lock|Foc:RetryHold|Foc:TakeoverDelta|If:Fallback|Sync:",
    mute: EC_MUTE,
    plot: ["Sync:Lock", "Sync:LockPeak", "Sync:ErrPeak", "Foc:LockErr", "Obs:OmegaHat",
      "Foc:TakeoverDelta"], big: ["Sync:Lock"],
    text:
      ...
      + "obserwatora."
      + "\n"
      + "Ustaw Foc:EntryFreq na punkt przejęcia wybrany podczas strojenia obserwatora. "
      + "If:FallbackFreq ustaw wyraźnie niżej, lecz powyżej dolnej częstotliwości pracy "
      + "wynikającej z Speed:Min. Pamiętaj, że Speed:Min ma jednostkę rpm: częstotliwość "
      + "elektryczna w Hz to rpm × Motor:PolePairs / 60. Odstęp między progami zapobiega "
      + "ciągłemu przełączaniu między I/f a FOC. Oprogramowanie napędu koryguje zbyt "
      + "mały odstęp, dlatego odczytana wartość Foc:EntryFreq może się różnić od wpisanej. "
      + "Nawet gdy wartość zadana jest niższa, napęd najpierw osiągnie Foc:EntryFreq, "
      + "zamknie pętlę i dopiero potem zejdzie do celu."
      + "\n"
      + "If:FallbackHigh określa górny próg powrotu do I/f: dopuszczalną nadwyżkę "
      + "częstotliwości obserwatora względem rampy. Ustaw go powyżej dodatnich odchyłek "
      + "widocznych w Obs:Bias i pozostaw zapas na wahania chwilowe. W przeciwnym razie "
      + "prawidłowa praca może zostać uznana za regenerację i pętla wróci do I/f bez "
      + "zgłoszenia błędu."
      + "\n"
      ...
      + "uśrednioną i może wyglądać poprawnie mimo chwilowych skoków błędu."
      + "\n"
      ...
      + "częstotliwości. Napęd porównuje chwilową prędkość obserwatora wraz z jej "
      + "tętnieniem, a Obs:Bias pokazuje wartość uśrednioną. Okno musi być więc szersze, "
      + "niż sugeruje ten odczyt. Jeżeli Sync:Lock nie dochodzi do końca, mimo że "
      + "Sync:ErrPeak jest mniejszy niż Foc:LockErr, a rampa zatrzymała się na "
      + "Foc:EntryFreq, najpierw poszerz Foc:LockSpeed. Zrób to przed zwiększeniem "
      + "Foc:LockErr. Nie rozszerzaj tolerancji wyłącznie po to, aby wymusić przejście "
      + "do closed. Ustawienie "
      + "Foc:LockErr na zero wyłącza przejęcie: napęd nie zatrzymuje się wtedy na "
      + "Foc:EntryFreq i pracuje dalej z wymuszonym kątem."
      + "\n"
      + "Foc:TakeoverDelta to kąt, o jaki pętla obróciła wektor w chwili przejęcia, "
      + "czyli kąt obciążenia odziedziczony z I/f. Odczyt zbliżony do 90° oznacza mały "
      + "zapas momentu i ryzyko niestabilnego przejęcia. Pomaga wtedy zwiększenie "
      + "odpowiedniego punktu tabeli Curr, a nie poszerzenie bramki przejęcia."
      + "\n"
      + "Po powrocie do I/f Sync:ExitCause podaje przyczynę, a Sync:ExitTime czas od "
      + "przejęcia do powrotu. low oznacza częstotliwość obserwatora poniżej "
      + "If:FallbackFreq, natomiast chatter oznacza ten sam warunek bezpośrednio po "
      + "przejęciu. Tak "
      + "szybki spadek zwykle wskazuje na utratę kąta przez obserwator, a nie na nagłe "
      + "zatrzymanie wirnika. high oznacza przekroczenie częstotliwości rampy o więcej "
      + "niż If:FallbackHigh, unlock — błąd kąta ponad Foc:LockErr, weak — zbyt mały "
      + "strumień, a config — nieprawidłową konfigurację obserwatora. Foc:RetryHold "
      + "blokuje kolejną próbę przejęcia przez zadany czas po każdym powrocie, aby "
      + "zapobiec szybkiemu przełączaniu między I/f a FOC."
      + "\n"
      + "Foc:EntryGlide wydłuża przejście składowych id i iq od wartości odziedziczonych "
      + "z I/f do wartości zadanych przez pętlę zamkniętą. Zwiększ tę wartość, gdy "
      + "przejęciu towarzyszy skok prądu albo zadziałanie zabezpieczenia peak. Kosztem jest "
      + "wolniejsza odpowiedź bezpośrednio po przejęciu."
      + "\n"
  ...
  { id: "foc", label: "6 FOC",
    ...
    text:
      ...
      + "przekraczaj 90%. Flaga sat oznacza osiągnięcie tego limitu. W pracy ustalonej "
      + "nie jest sama w sobie błędem, ale jej pojawienie się w chwili przejęcia oznacza, "
      + "że pętla nie ma zapasu napięcia potrzebnego do regulacji."
      + "\n"
      + "Foc:Flags utrzymuje przez około 2 s informację o krótkotrwałych zdarzeniach: "
      ...
      + "Obs:Bias."
      + "\n"
      + "Jeżeli zaraz po przejęciu rośnie napięcie szyny i zadziała zabezpieczenie hv+, "
      + "oznacza to, że pętla prędkości hamuje wirnik i oddaje energię do szyny. "
      + "Zmniejsz SpeedCtrl:Kp "
      + "i SpeedCtrl:Ki, wydłuż Foc:EntryGlide, a przy hamowaniu poszerz "
      + "Brake:RegenBand."
      + "\n"
